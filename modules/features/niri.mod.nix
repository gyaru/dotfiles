{self, ...}: {
  flake.modules.hjem.niri = {
    ...
    pickColor = getExe self.packages.${pkgs.stdenv.hostPlatform.system}.niri-pick-color;
    niri = getExe pkgs.niri;
    orderOutputs =
      pkgs.writeShellScript "niri-output-order"
      /*
      bash
      */
      ''
        # SDL2 USES WAYLAND OUTPUT ANNOUNCEMENT ORDER
        if ${niri} msg --json outputs | ${getExe pkgs.jq} --exit-status '."DP-2".logical != null' > /dev/null; then
  ...
  in {
    ...
    rum.programs.zsh.loginConfig =
      /*
      zsh
      */
      ''
        if [ -z "$WAYLAND_DISPLAY" ] && [ "$XDG_VTNR" -eq 1 ]; then
          exec ${getExe' pkgs.dbus "dbus-run-session"} ${niri}
        fi
      '';
    ...
    rum.desktops.niri = {
      ...
      extraVariables.SDL_VIDEO_DISPLAY_PRIORITY = "DP-2";

      spawn-at-startup = [
        (singleton "${orderOutputs}")
